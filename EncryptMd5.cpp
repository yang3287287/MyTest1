// EncryptMd5.cpp: implementation of the CEncryptMd5 class.
//
//////////////////////////////////////////////////////////////////////
...
*****************************************************************************************/
CString CEncryptMd5::GetMD5(BYTE* pBuf, UINT nLength)
{
	//entry invariants
//	IsValidAddress(pBuf,nLength,FALSE);

	//calculate and return the checksum
	CEncryptMd5 MD5Checksum;
	MD5Checksum.Update( pBuf, nLength );
	return MD5Checksum.Final();
}
CString CEncryptMd5::GetMD5(CString &sInput)
{
	CStringA sInputA(sInput);
	CEncryptMd5 MD5Checksum;
	MD5Checksum.Update((BYTE*)sInputA.LockBuffer(), sInputA.GetLength());
	return MD5Checksum.Final();
}

CString CEncryptMd5::GetMd5File(CString filename,ULONGLONG&filelen)
{
	CString strmd5;
	CFile File;
	if(File.Open(filename, CFile::modeRead | CFile::shareDenyNone | CFile::typeBinary,NULL))
	{
		CEncryptMd5 MD5Checksum;		//checksum object	
		int nLength = 0;				//number of bytes read from the file
...
void CEncryptMd5::HexStringToBin(char*hexstring,BYTE*value)
{
	for(int i=0;i<16;i++)
	{
		char buff[4];
		memset(buff,0,4);
		char*p = hexstring+i*2;
		memcpy(buff,p,2);
		sscanf(buff,"%X",&value[i]);
	}
}
...
*****************************************************************************************/
DWORD CEncryptMd5::RotateLeft(DWORD x, int n)
{
...
NOTES:			None
*****************************************************************************************/
void CEncryptMd5::FF( DWORD& A, DWORD B, DWORD C, DWORD D, DWORD X, DWORD S, DWORD T)
{
	DWORD F = (B & C) | (~B & D);
	A += F + X + T;
	A = RotateLeft(A, S);
...
NOTES:			None
*****************************************************************************************/
void CEncryptMd5::GG( DWORD& A, DWORD B, DWORD C, DWORD D, DWORD X, DWORD S, DWORD T)
{
	DWORD G = (B & D) | (C & ~D);
	A += G + X + T;
	A = RotateLeft(A, S);
...
NOTES:			None
*****************************************************************************************/
void CEncryptMd5::HH( DWORD& A, DWORD B, DWORD C, DWORD D, DWORD X, DWORD S, DWORD T)
{
	DWORD H = (B ^ C ^ D);
	A += H + X + T;
	A = RotateLeft(A, S);
...
NOTES:			None
*****************************************************************************************/
void CEncryptMd5::II( DWORD& A, DWORD B, DWORD C, DWORD D, DWORD X, DWORD S, DWORD T)
{
	DWORD I = (C ^ (B | ~D));
	A += I + X + T;
	A = RotateLeft(A, S);
...
*****************************************************************************************/
void CEncryptMd5::ByteToDWord(DWORD* Output, BYTE* Input, UINT nLength)
{
	//entry invariants
//	ASSERT( nLength % 4 == 0 );
...
*****************************************************************************************/
void CEncryptMd5::Transform(BYTE Block[64])
{
...
*****************************************************************************************/
void CEncryptMd5::DWordToByte(BYTE* Output, DWORD* Input, UINT nLength )
{
	//entry invariants
//	ASSERT( nLength % 4 == 0 );
...
*****************************************************************************************/
void CEncryptMd5::Update( BYTE* Input,	ULONG nInputLen )
{
